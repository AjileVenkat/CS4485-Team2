...
X_best = select.transform(X)
cv_scores = cross_val_score(optimal_random_forest, X_best, y, cv=skf)

print(f"Optimal Parametesr: {search.best_params_}")
# Display performance metrics (will add more metrics for further evaluation)
y_predict = optimal_random_forest.predict(X_test_best)
...
print(f"Best features: {list(select_names)}")
